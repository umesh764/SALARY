...
def calculate_salary():
    try:
        ...
        conveyance = round(adjusted_basic_salary * 0.0, 2)  # Conveyance 0% of Adjusted Salary (change this if needed)
        ...
        leave_deduction = round(gross_salary * 0.0125, 2)  # 1.25% of Gross Salary
        
        # Cap Leave Deduction
        if leave_deduction >  00:
            leave_deduction = 00
        ...
        city_allowance_percentage = random.uniform(0.60, 0.61) if emp_name not in employees_reduced_city_allowance else random.uniform(0.60, 0.61)
        ...
        esic = round(adjusted_basic_salary * 0.0805, 2)

        # Total Emoluments (TEC)
        total_emoluments = gross_salary + city_allowance + bonus + leave_deduction + epf + professional_tax + gratuity + provident_fund
        ...
        take_home_salary = gross_salary + city_allowance - total_deductions + epf + bonus + professional_tax
        ...
        # Subtract the advance deduction from the take-home salary
        final_take_home_salary = take_home_salary - advance_deduction
        ...
        if messagebox.askyesno("Export", "Do you want to save this data to an Excel file?"):
            save_to_excel(company_name, month, post, emp_name, adjusted_basic_salary, hra, conveyance, city_allowance, epf, provident_fund, gratuity, leave_deduction, bonus, professional_tax, esic, final_take_home_salary, total_emoluments_12_months)
    except Exception as e:
        messagebox.showerror("Error", f"Calculation error: {str(e)}")

# Function to save data to Excel
def save_to_excel(company_name, month, post, emp_name, basic_rate, hra, conveyance, city_allowance, epf, provident_fund, gratuity, leave_deduction, bonus, professional_tax, esic, take_home_salary, total_emoluments_12_months):
    wb = Workbook()
    ws = wb.active
    ...
    data = [
        ...
        ("Conveyance (0% of Basic Pay)", conveyance),
        ...
        ("Advance Deduction", entry_advance.get()),
...
entry_conveyance = create_label_entry_right("Conveyance (0% of Basic Pay):", 3, 2)
...
entry_esic = create_label_entry_right("ESIC (8.05% of Basic Pay):", 11, 2)
entry_take_home = create_label_entry_right("Take Home Salary:", 12, 2)
entry_total_emoluments = create_label_entry_right("Total Emoluments (TEC):", 13, 2)

entry_days_worked = tk.Entry(root, font=("Arial", 10), width=20)
entry_days_worked.grid(row=14, column=1, padx=10, pady=5)

entry_advance = tk.Entry(root, font=("Arial", 10), width=20)
entry_advance.grid(row=15, column=1, padx=10, pady=5)

# Calculate and Export buttons
button_calculate = tk.Button(root, text="Calculate Salary", bg="#4CAF50", fg="white", font=("Arial", 12), command=calculate_salary)
button_calculate.grid(row=16, column=1, padx=20, pady=10)
